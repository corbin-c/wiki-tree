...
onmessage = function(event) {
  if (typeof event.data.parameters !== "undefined") {
    ...
    var simulation = d3.forceSimulation(event.data.tree.nodes)
      .force("link", d3.forceLink(event.data.tree.links).id(function(d) { return d.id; }).distance(60*k))
    ...
    postMessage({id:event.data.id, nodes: event.data.tree.nodes, links: event.data.tree.links});
  }
}
